...
#define FNPC_FLAG_HEAL_AFTER_FOLLOWER_BATTLE      TRUE     // Replace the TRUE with a flag in order to use that flag to toggle whether the Player's party will be automatically healed after every follower partner battle. If you want this to always be active without using a flag, replace TRUE with FNPC_ALWAYS.
#define FNPC_FLAG_PARTNER_WILD_BATTLES            TRUE     // Replace the TRUE with a flag in order to use that flag to toggle whether the follower partner will join you for wild battles. If you want this to always be active without using a flag, replace TRUE with FNPC_ALWAYS.
